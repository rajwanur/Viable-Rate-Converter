...
const fetchFromExchangeRateApi = async (): Promise<Rates> => {
  ...
  if (!rates || !rates.MYR || !rates.BDT || !rates.EUR) {
    throw new Error(
      "Required currency rates (MYR, BDT, EUR) not found in API response.",
    );
  }
...
const fetchFromWiseApi = async (apiKey?: string): Promise<Rates> => {
  ...
  if (!apiKey) {
    throw new Error(
      "A Wise API key is required. Please enter your key to fetch rates from Wise.",
    );
  }
  ...
  const fetchRateWithKey = async (source: string, target: string) => {
    ...
    if (response.status === 401 || response.status === 403) {
      throw new Error(
        `Wise API authentication error (${response.status}): Please check your API key.`,
      );
    }
    if (!response.ok) {
      throw new Error(
        `Wise API error for ${source}->${target}: ${response.statusText}`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Invalid data structure from Wise API for ${source}->${target}`,
      );
    }
    return data[0].rate;
  };

  const [gbpToMyr, gbpToBdt, gbpToEur, myrToBdt, eurToBdt] = await Promise.all([
    fetchRateWithKey("GBP", "MYR"),
    fetchRateWithKey("GBP", "BDT"),
    fetchRateWithKey("GBP", "EUR"),
    fetchRateWithKey("MYR", "BDT"),
    fetchRateWithKey("EUR", "BDT"),
  ]);
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message === "Failed to fetch") {
        throw new Error(
          `Network error while fetching from ${source}. Please check your connection.`,
        );
      }
      throw error;
    }
