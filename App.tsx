...
const App: React.FC = () => {
  ...
  const [apiSource, setApiSource] = useState<ApiSource>(
    ApiSource.EXCHANGE_RATE_API,
  );
  const [wiseApiKey, setWiseApiKey] = useState<string>(
    () => localStorage.getItem("wiseApiKey") || "",
  );
  ...
  const { conversionResults, marketMyrBdtRate } = useMemo(() => {
    ...
    if (
      ...
      let defaultOtherCurrency: Currency;
      if (inputCurrency === Currency.GBP) {
        defaultOtherCurrency = Currency.MYR;
      } else if (inputCurrency === Currency.MYR) {
        defaultOtherCurrency = Currency.GBP;
      } else {
        defaultOtherCurrency = Currency.GBP;
      }
      ...
      return {
        conversionResults: {
          otherCurrency: defaultOtherCurrency,
    ...
    if (inputCurrency === Currency.GBP) {
      ...
      eurAmount = gbpAmount * rates.GBP_TO_EUR;
    } else if (inputCurrency === Currency.MYR) {
      myrAmount = numericAmount;
      gbpAmount = myrAmount / rates.GBP_TO_MYR;
      eurAmount = gbpAmount * rates.GBP_TO_EUR;
    } else if (inputCurrency === Currency.EUR) {
      eurAmount = numericAmount;
      gbpAmount = eurAmount / rates.GBP_TO_EUR;
      myrAmount = gbpAmount * rates.GBP_TO_MYR;
    }

    const bdtAmount = gbpAmount * rates.GBP_TO_BDT;

    const incentivePercentage = incentiveEnabled
      ? parseFloat(incentiveRate) / 100
      : 0;
    const validIncentive =
      !isNaN(incentivePercentage) && incentivePercentage > 0;
    const bdtWithIncentive =
      bdtAmount * (1 + (validIncentive ? incentivePercentage : 0));

    const calculatedMyrBdtRate =
      myrAmount > 0 ? bdtWithIncentive / myrAmount : 0;

    // For now, show the other two currencies as additional conversion results
    // The primary "other currency" logic shows GBP ↔ MYR, but we'll handle EUR differently
    let otherCurrency: Currency;
    let otherCurrencyAmount: number;

    if (inputCurrency === Currency.GBP) {
      otherCurrency = Currency.MYR;
      otherCurrencyAmount = myrAmount;
    } else if (inputCurrency === Currency.MYR) {
      otherCurrency = Currency.GBP;
      otherCurrencyAmount = gbpAmount;
    } else {
      // For EUR, show GBP as the primary other currency
      otherCurrency = Currency.GBP;
      otherCurrencyAmount = gbpAmount;
    }
  ...
  const bdtCardTitle =
    incentiveEnabled && !isNaN(incentiveDisplayRate) && incentiveDisplayRate > 0
    ...
    <div className="min-h-screen bg-gray-900 text-gray-200 flex flex-col items-center p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-lg mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <ResultCard
                        ...
                        amount={formatCurrency(
                          conversionResults.otherCurrencyAmount,
                          conversionResults.otherCurrency,
                        )}
                      ...
                      <ResultCard
                        title={bdtCardTitle}
                        amount={formatCurrency(
                          conversionResults.bdtWithIncentive,
                          Currency.BDT,
                        )}
        ...
        <footer className="text-center mt-8 text-gray-500 text-sm">
          ...
          <p>
            &copy; {new Date().getFullYear()} Viable Rate Calculator. All rights
            reserved.
          </p>
        </footer>
      </div>
